...
- **P0 — Evidence audit (device, read-only, "before" column):** for each of the six rows via card menu → Details → source email: From, subject, date, amount in email, bill no., payment method, current row state. Play receipts from corpus. Phase 0 was started 2026-09-17 and abandoned in the vault disaster; unblocked after corpus restore. Docs hygiene happens only AFTER the gate passes — never instead of it.
