...
- **L2 — cid inline-part plumbing (queued directly after A, plan agreed 2026-09-15):** the corpus's zero-seed restage (938 candidates, zero `emailIconUrls`) + C1's deferred cid tranche. Step 0 probe scan (PARSER_VERSION 17 + temp `[L2-PROBE]` diag): does proton/imap `text` carry image parts (Content-ID + base64)? JS MIME splitter `extractMimeParts` → `inlineParts {contentId, format, base64}` (≤4, ≤300KB, image/*+svg) wired into proton+imap normalizers as in-memory `NormalizedMessage.inlineParts`; extractor cid branch emits `scanblob:<sha256>` ranked FIRST (strongest provenance) else records-and-skips as today; `classifyMessage` persists emitted blobs to new `scan_blobs(hash, format, bytes_b64, created_at)` (schema MIGRATIONS bump; purge >7d or >4MB); `fetchAndSaveUrl` gains the `scanblob:` branch (read → validate → same promote path; skip markUrlAsCrawled); format sniff at both seed sites uses stored format. Probe decides branch: if Proton body lacks image parts → branch B = native `/mail/v4/attachments/{id}` + session-key decrypt. Tuta file facade + gmail-attachment route explicitly deferred (tranche 3) pending probe evidence. Gate: `[EMAIL-DIRECT] trying N` with `scanblob:` seeds → `card icon applied from email` → visual Home-card check via uiautomator dump (screencap untrusted).
