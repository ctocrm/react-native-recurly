...
**DONE 2026-09-12 (device gate this session — verification-only, build unchanged).** Real `[RDAP] lookup` lines for **3 distinct domains** in `/tmp/phaseC_gate.log`, all `corroborates=yes`: **zohoaccounts.ca** (`org="Zoho Canada Corporation"` — org-token match, the strongest corroboration path), **zerosupporthosting.ca** (registry answered with no org → From-host fallback; dead-site path proven: TIER 0.5 scrape failed `Network request failed` exactly as expected), **x.ai** (`org="DNStination Inc."` — real registry answer behind a privacy proxy → From-host fallback; non-.com ccTLD bootstrap). Negative case proven first (Proton: empty TIER 0 discovery → **no** RDAP line — resolution gated on discovery by design). Full pipeline visible around the RDAP lines: x.ai TIER 0.5 extracted 12 icon URLs from the live site; TIER 3 provenance filters rejected 40 untrusted + 2 junk-farm + 3 brand-token-no-signal; a downloaded empty/transparent icon was rejected post-fetch; a wrong-company lookalike domain (xai.ma) passed domain-token filtering but was flagged Wrong by brand classification (G-class quality, not Phase D scope). **One substitution vs the gate letter:** EMAIL-SEEDS contains exactly 3 domains (zohoaccounts.ca, zerosupporthosting.ca, x.ai) — no .io/.co seed exists, so the oddball slot ran as **.ai** (same non-.com IANA-bootstrap class). Accept-the-substitution or add-one-.io/.co-seed-and-rerun is the user's call. Full evidence in the changelog row.
