...
**DONE 2026-09-12 (`d006703`, device-gated same session).** Implementation: file-level `unlockedScopes` ConcurrentHashMap (uid → accessToken) in `ProtonModule.kt` — file scope is REQUIRED because `ProtonClient` is re-instantiated per `listWithSession` call, so any instance field would not survive batches. Guard at the top of `unlockLockedScope` (cache hit logs `locked-scope unlock cached (P3) — skipping SRP reauth for this scan`), cache set only after the `users/unlock` PUT succeeds, so a failed SRP unlock never poisons the cache. Self-invalidating on the P2 refresh path: the key contains the current accessToken, and a refresh mints a new one → next batch re-runs the SRP unlock. Per-batch key derivation (`unlockKeys` users/addresses/salts fetch + armored-key unlock) intentionally UNCHANGED — the board specced the SRP reauth skip only. C2: tsc 0, jest 286/286 (native change has no jest coverage — the live device gate is the proof, stated honestly). C3: `./scripts/android/build-android.sh --dev`, `BUILD SUCCESSFUL in 3m 58s` (warm daemon; trailing Metro step skipped, 8081 held by the standing server). C4 gate PASSED with the mailbox grown since the recipe was written (475 messages / 7 pages, not 451): **unlock ok == 1**, **cached == 6** (batches 2–7), `listed 75`×6 + `listed 31` = 475 of 475, every batch `fail=0`, `batches staged, messages=475 (chunk=75)`, zero `PROTON_ABUSE`/2028/429/CAPTCHA (apparent grep hits were frame-time stats and accessibility IDs), Monthly Spend stable $588.55 (recipe's $291.45 is stale calibration — superseded by later rescans; current baseline held). One P2 reactive refresh fired at 22:56:33 BEFORE batching (`access token rejected — refreshing session`) and the new token re-keyed the cache exactly as designed — first batch unlocked once, remaining six reused. Evidence: `/tmp/p3_gate.log`. Ops lessons this session: after a rebuild+reinstall the app can red-screen on `loadJSBundleFromAssets` — fix is `adb reverse tcp:8081 tcp:8081` + force-stop/relaunch (the tunnel drops when adb reconnects); the emulator's tab bar occupies y≈2114–2240 and eats injected taps — scroll targets clear of that band and re-dump before tapping (moving-target taps caused four dead taps on the Clear Email Scan Cache button). **Next: Phase D (P4 official client fingerprints) — the remaining reauth-hardening hop.**
...
**CLOSED 2026-09-13 (`330f65c` + `e5cae32` + `0e32c45`) — landed as PARTIAL vs the original letter, with the negative result conclusively proven empirically.** The doc records NO literal official-web appversion/UA strings (D1's premise was wrong — §D5 only documents our old values), so the doc's own prescription governed: `cadence@<version>` + stable UA. Empirical gate runs forced two corrections, each certified by the server itself: `cadence@1.0.0` → **HTTP 400 Code 2064** "platform and product must be separated by a dash"; `android-cadence@1.0.0` → **2064 "Product `cadence` is not valid"** — Proton ALLOWLISTS products in `x-pm-appversion`; third-party apps cannot register one, so honest self-identification via appversion is impossible (spoofing official client strings was considered and rejected as out of scope without an explicit user order). **Final landed state:** `x-pm-appversion` stays `"Other"` (the server-accepted value for unknown clients — every historical successful scan used it) + `User-Agent: Cadence/1.0.0` (stable, honest, names the product; UA is not allowlist-validated) — one constants block citing the doc + both 2064 evidences, all three header sites (postJson/putJson/getJson). D3 gate PASSED on the final state (`0e32c45`, scan 05:32–): listed **7/7** pages (75×6+31 = **475 of 475**), unlock **ok==1 + cached==6** (P3 intact), every batch `fail=0`, `batches staged, messages=475`, **zero** 2064/PROTON_ABUSE/2028/9001/429/CAPTCHA in the gate window, no reconnect prompt. tsc 0 + jest 286/286 (native change has no jest coverage — device gate is the proof). The two 2064 rejections were themselves clean failures (typed `MailScanUnverifiedError`, leg failed gracefully, scan continued) — the truthful-error path (P1) held. **Net vs D5:** fingerprint risk surface unchanged on appversion (impossible to improve honestly), UA improved from a stale personal string to a stable product identity; the remaining D5 items (HTTP/1.1-only HttpsURLConnection TLS fingerprint, emulator/datacenter IP) are unaddressed — OkHttp/HTTP2 remains future-optional per the doc. Evidence: `/tmp/p4_gate.log` (both 2064 rejections), `/tmp/p4_gate2.log` (clean gate scan). **Reauth-hardening arc P1+P2+P3+P4 now complete.**

### Phase E — I3 report-stick live exercise (behavioral proof, no code)
**DONE 2026-09-11 (docs-only; no code change — see changelog row for full evidence).** E1 promote-path located (`iconBackgroundCrawler.ts:1553-1575`; decisive log signature: `No valid icons to auto-assign` — `canAutoAssignCache` returns silently earlier, so that line's presence proves the report filter ran). E2 tsc 0 + 242/242 jest. E3 PASS: linode run — report filed → re-crawl → 16× `Skip downgrade for linode`, ZERO promote lines, card icon identical A vs B (`/tmp/e3_linode_A.png` → `/tmp/e3_linode_B.png`); openai run — 4× `No valid icons to auto-assign for openai` proves the reported hash was filtered, the later assign was a different non-reported candidate (B≠A there, documented). NEW BUG logged: "Show incorrect/Broken" reveals are dead (stale closure → "Mark as good" unreachable). **FIXED + LIVE-VERIFIED 2026-09-13 (`7199efb` structural fix + `6f860c8` submitReport completion) — two stacked defects:** ① stale closure/load-time filtering (`7199efb`): `availableIcons` stores the FULL mapped collection, visibility derives at render (`visibleIcons` useMemo over reportedType × showIncorrect/showBroken), switches bind `onValueChange={setShow*}` directly, `handleMarkAsGood` flips the tile in place. ② found only by same-day live re-test: `submitReport` still REMOVED the tile (pre-refactor leftover), making the toggles dead for any icon reported in that session — fixed by marking `reportedType` in place (`6f860c8`). Device-verified end-to-end on linkedin (2-icon collection, embedded 09:45 build): Wrong+comment → "1 icon available"; Show incorrect ON → "2 icons available" + ✓ Good chip; ✓ Good → "Restored"; toggle OFF → still 2 (restored tile persists; remove-behavior would have dropped it). Env notes: `getIconCollection` runs a ~44s heal on open and rewrites collections between sessions (linkedin 4→2; zoho/zerosupporthosting drained to 0) — reports can orphan harmlessly when referenced bytes are replaced (QA test rows noted); post-reboot emulator input changed mechanics (motionevent DOWN/UP long-press stopped registering; same-point `input swipe X Y X Y 900` works).
...
Scan-wide budget + per-leg progress lines (`[MailScan] outlook 210/314 …` every 30s). Unit-test the pacing pure function; gate = one healthy-link scan with ≥1 progress line per leg, total <5 min. **DONE 2026-09-14 (`513245f`; pacer itself landed in GOLD BASELINE `42a7435`)** — `legProgress.ts` (30s pacer + terminal line, injectable clock) + `scanTotalBudget.ts` (10-min scan-wide budget, checked BETWEEN legs only: in-flight leg never killed, watchdog stays the silence authority, `endScan()` invariant untouched). Gate PASSED live: steady-state scan 3m51s (<5 min), 4/4 legs ≥1 progress line + terminal `leg done` line, zero budget lines (healthy absence), zero stalls, zero interventions. One-time PARSER_VERSION-13 re-list measured 17m14s (932 msgs restaged) — observable throughout, self-terminating, no trickery. jest 302/302 (36 suites), tsc 0, eslint 0.

### Phase I — background scan UX (user directive 2026-09-14: "let it run in the background and async, everything should just show up on the UI cards, icons, prices as the user uses the app")

Current state: results DO land live per leg and icons drain async AFTER a scan, but the scan presents as blocking (spinner + disabled button + Alert dialog) and all progress is logcat-only. Phase I makes the scan non-blocking for real:

- I1: scan-progress event surface from the leg loop (legs done/total, staged counts via the existing pacer ticks) + `useScanProgress` hook. **DONE 2026-09-14 (`2fb1278`)** — `scanProgress.ts` external store wired into the leg loop (legStart/tick/legDone/legError with double-count guard/finish) + `useScanProgress` (useSyncExternalStore).
- I2: Home scan button goes fire-and-forget (no full-time spinner/disable); persistent "Scanning… leg k/N" pill; completion toast instead of `Alert.alert`. **DONE 2026-09-14 (`2fb1278`)** — button renders live `Scanning… leg k/N` from the store (survives navigation), tap during an active scan is a no-op, success completion is a toast (system Toast-window receipt 18:39:23). Gate PASSED: Home→Subscriptions→Home mid-scan, pill persisted and advanced to `leg 4/4`, app usable throughout, post-scan audit `match=yes`.
- I3: per-mailbox errors surface without blocking (R2 parity). **DONE (impl)** — errors ride the store and the completion Alert (detail path); not exercised live (steady-state scan had zero errors).
- I4 (separately gated): icons-during-scan — only after re-proving the OOM ceiling that motivated Gate A (2026-09-05 192MB crash); else Gate A parking stays. **NOT ATTEMPTED.**
- Gate: user-path — tap scan, navigate tabs while it runs, cards land per leg, pill tracks legs, app usable throughout; release-build memory check if I4 attempted. One phase per session — do not mix with other work. **GATE PASSED 2026-09-14.**
...
**J1 — F-6 spend integrity (first).** `spend-projection-audit: projection=669.31 legacy=519.31 delta=150.00 match=NO` (2026-09-14, after scan #1's 59 workspace candidates; Home renders $669.31 vs $599.68 pre-scan; prior sibling: 09-12 $588.55→$438.55 "audit item ①"). J1a recompute the projection fold and the legacy fold over current rows; identify the diverging row/class ($150.00 exact — hypotheses: a yearly row's monthly-contribution treatment diverges between projection buckets and legacy fold; sparse/recurring split from the workspace candidates; `both`-bucket overlap correction). J1b fix + extend the projection≡legacy fixture suite with the failing case. J1c gates: unit equality green; device boot audit `match=yes`; Home number explainable card-by-card; survives a re-scan. **Named outcome: Home's Monthly Spend provably equals the row data.** **DONE 2026-09-14 (`6fa33dd`) — root cause was snapshot staleness, not the fold: fresh boot read both sides at 519.31 (delta 0.00, match=yes); the hook kept boot-time buckets (581 pre-scan rows) while imports rebuilt the projection (191 buckets). The $150 shift itself is the legitimate PARSER_VERSION-13 re-classification (sparse −150.00, recurring +69.63, net 599.68→519.31). Fix: `onProjectionRebuilt` event + `useChargeDisplay` reloads actuals on every rebuild. Gate PASSED: scan #3 (delta) drove rebuild→reload→audit `match=yes` mid-scan after every leg AND post-scan without restart; Home renders $519.31 = both folds. jest 305/305 (36 suites), tsc 0, eslint 0.**

**J2 — Gate A starvation fix.** A discovery flow started just before a scan (fastbtc, 2026-09-14) runs INTO the scan and starves legs — Gate A pauses the queue drain but not in-flight discovery flows; a 3-message workspace leg took ~2.5 min and nearly drew a false watchdog kill. J2a make scan-start pause (or abort-and-requeue) in-flight discovery stages: move the `waitIfScanActive` gate inside the discovery loop, or have `startIconCrawl` abort-and-requeue when `isScanActive()` flips. J2b gates: unit test for the pause; device repro of the fastbtc case (start a crawl, immediately scan) — leg timings unaffected, no false watchdog kill. **DONE 2026-09-14 (`3442fea`)** — `searchAllSources`/`searchForLinksToSpider` accept a stage `gate` (awaited at DDG/Bing/dorks boundaries and before the spider fetch); the crawler passes `waitIfScanActive` and also parks before TIER-2 immediate fetches. Device repro PASSED with a paired-scan: parked crawls (hangtag, hflamtl) held their searches through scan A and scan B respectively — hflamtl's TIER-3 storm began 274ms after `scan ended`; scan B ran 40s total with fast legs and zero stalls; parked work resumed in queue order with nothing dropped. jest 307/307 (37 suites), tsc 0, eslint 0. Residual (documented, not taken): TIER-2 favicon fetches can still overlap if the flow passed its gate pre-scan — bounded and small.
...
**DONE 2026-09-14 (docs-only close; zero code changes).** 6-step reveal-toggle protocol + G1 brand-grid gate re-run PASS on current HEAD (4 pickers incl. the F-2-renamed Zoho; zero junk in any grid); baseline row 15 reconciled against the 09-13 closure (`6f860c8` — the protocol had already run once); zohoaccounts "flagged for D/E" closed as mooted by F-2 (Phase C row annotated); opportunistic email-seed apply protocol documented at the C4 gate below. Device state unchanged (subs=124, $519.31, no popup). Receipts: `docs/baselines/2026-09-14.md` J4 section. **Phase J complete.**
...
- C4 gate: build + one scan (or picker "Search for Icon Online" on a fresh 0-icon sub if scan won't fire crawls — G0 premise) showing the seed fetch in the log and ≥1 brand-correct apply sourced from the email. Docs row + commit.
  - J4 documentation (2026-09-14): the opportunistic email-seed VISUAL apply protocol — run C4 on the NEXT NATURAL 0-icon sub whose email-sent icon host is ALIVE (zerosupporthosting can never demonstrate it: user-confirmed dead 2026-09-12, and the J5 liveness short-circuit now skips it in ~0.6 s with zero network). Steps: (1) confirm the host is alive BEFORE any device work (`curl -sI` from the host — zerosupporthosting precedent: it refused every request from both host and emulator); (2) prefer the picker-fired single-brand crawl ("Search for Icon Online") over a full scan — one brand, one crawl (J5 revised-protocol precedent), no cache clear; (3) ordered log witnesses: `[SEARCH] EMAIL-SEEDS: queued N of M brand-sent URL(s) for <key>` (queued before every network tier) → `[FETCH] Immediate fetch: email_logo <url>` (rank-5 provenance outranks web discovery) → download/save lines → `[CRAWL] Auto-assigned best valid icon for <key>`; (4) PASS = ≥1 brand-correct apply SOURCED FROM EMAIL (card visual + `email_*` provenance in the crawl rows); (5) no build/rebuild needed (mechanism shipped in `e1d16b5`, device-proven to the fetch step 2026-09-12).

### D — RDAP resolver (domain→org evidence for brand-correctness)

**DONE 2026-09-12 — gate letter amended by registry fact.** Real `[RDAP] lookup` lines for **4 distinct domains** in `/tmp/phaseC_gate.log`: **zohoaccounts.ca** (`org="Zoho Canada Corporation"` — org-token match, strongest path), **zerosupporthosting.ca** (registry answered, no org → From-host fallback; dead-site chain correct), **x.ai** (`org="DNStination Inc."` — privacy proxy → From-host fallback), **cline.bot** (real .bot registry answer via IANA bootstrap→Nominet; `org="?"`, `corroborates=no` honestly — curated-seeded crawl has no From-host, and RDAP never auto-replaces on uncorroborated evidence; TIER 0.5 then extracted 24 icon URLs from the seeded official site). Negative case proven (Proton: no discovery → no RDAP). **The gate letter's ".io/.co oddball" is unsatisfiable as written: `.io` and `.co` are ABSENT from the IANA RDAP bootstrap** (verified 2026-09-12: rdap.org returns 404 "No RDAP service" for wert.io; `data.iana.org/rdap/dns.json` contains no `io`/`co` entries — checked alongside ai/bot/app/dev which ARE present). The oddball requirement is met in substance by **.ai** and **.bot** — same non-.com IANA-bootstrap class, with real registry answers on device. Code this session: curated `KNOWN_OFFICIAL_DOMAINS` seed map in `officialDomain.ts` (`wert`→wert.io, `cline`/`clinebotinc`→cline.bot — entries must be the brand's genuine official host, verified live before adding), wired as a TIER 0 branch after session-seed/compound-slug, persisting into `icon_crawl_sessions.official_domain` (`1f339a0`, `40dbec2`); tsc/eslint clean, jest 286/286. Ops notes: the on-device DB (`files/SQLite/user_local.db`) is encrypted — seeds must enter via code or the scan; thewert.io RDAP absence is a registry fact, not a bug; picker deep-search keeps the JS thread busy enough to drop injected taps (wait for drain or use uiautomator-driven coordinates).
...
| 2026-09-12 | **PHASE C DEVICE GATE C4 — email-seed mechanism PROVEN live on device; email-sourced VISUAL apply UNMET (brand host dead — external).** Phase C code `e1d16b5` built + installed (`./scripts/android/build-android.sh --dev`, `BUILD SUCCESSFUL in 10m 29s`; only the trailing Metro step skipped — stale Metro held 8081; fresh Metro via `systemd-run --user --unit=metro-c2 --working-directory=<project> npx expo start --port 8081` — NOTE: transient units do NOT inherit the caller's cwd, the first attempt without `--working-directory` started expo in `$HOME` and died in 3s; app bundled 2226 modules from the fresh server). Fresh capture `/tmp/phaseC_gate.log`. **C4 setup via UI:** deleted 0-icon **zerosupporthosting** ($1.99 Monthly, "+" placeholder) via card ••• → Delete → confirm; Settings → Cache & Crawl Data → Clear Email Scan Cache **(2217 → 0)**; started "Scan mailbox for subscriptions". **Full re-scan clean:** Gmail listed **2095 ids / 21 pages / 1692 bodies** in ~41min (05:19:46 active → 06:03:00 ended), R12 pacing + watchdog quiet, `[SCAN_STATE] scan active (depth=1)` → `(depth=0)`. **Mechanism proof (the Phase C path, in order):** 06:03:06 `[SEARCH] ===== STARTING SEARCH for zerosupporthosting =====` → 06:03:10 **`[SEARCH] EMAIL-SEEDS: queued 1 of 1 brand-sent URL(s) for zerosupporthosting`** → 06:05:29 **`[FETCH] Immediate fetch: email_logo https://zerosupporthosting.ca/logo.png`** — the email-extracted seed was the crawl's FIRST fetch with `email_logo` provenance (rank-5 path), ahead of all web discovery, exactly as wired (`seedUrls` → `findIconUrls` → saveCrawlResult+urlsToFetch first). Zerosupporthosting re-imported NEW (UI-verified placeholder card, $1.99 sparse, after delete + cache clear). **Apply half NOT met — external blocker:** zerosupporthosting.ca refused EVERY request from BOTH the emulator and the host machine (`curl -sI https://zerosupporthosting.ca/logo.png` and `/favicon.ico` → empty responses from the host; emulator saw `Network request failed` through all retries) — dead/blocking brand server, so the email seed AND the web-discovered official favicon all failed and the search ended `[CRAWL] No valid icons to auto-assign for zerosupporthosting (0 with data, all invalid/empty)`. Only zerosupporthosting produced email seeds this scan (all other subs re-classified too, but none yielded qualifying brand-sent URLs / already had icons), so **"≥1 brand-correct apply sourced from email" stays OPEN** — not closable with this brand; needs a 0-icon sub whose email-sent icon host is alive (**user confirmed 2026-09-12: zerosupporthosting is their old company, now closed — the host is permanently dead; this brand can never demonstrate the visual apply; proof deferred to a live-host 0-icon sub**). zohoaccounts spot-check: icon unchanged (blue product mark, bing_images origin per Gate B log) — still flagged for D/E. [J4 2026-09-14: MOOTED by F-2 — the flag predated the identity fix; canonical Zoho end-to-end with the real 77,608-byte tile preserved; zoho picker grid 2/2 brand-correct on the HEAD re-run; receipts in docs/baselines/2026-09-14.md J4 section.] Monthly Spend $588.55 → $438.55 after this scan (re-opens audit item ①, own hop). Post-scan drain ran normally (24+ searches finished, zero mid-scan crawl starts — Gate A holding). Artifacts: `/tmp/phaseC_gate.log`, `/tmp/phaseC_build.log`, `/tmp/metro_c.log` (unit metro-c2). |
| 2026-09-12 | **PHASE D DEVICE GATE — RDAP resolver PROVEN live on device for 3 real domains, all `corroborates=yes`; negative case (no discovery → no RDAP) also proven. Oddball letter (.io/.co) SUBSTITUTED by .ai — no .io/.co seed exists (EMAIL-SEEDS = zohoaccounts.ca, zerosupporthosting.ca, x.ai); user to accept or order a one-seed hop.** Build unchanged from Phase C session (`e1d16b5` APK, RDAP/TIER-0 logging); fresh capture `/tmp/phaseC_gate.log`; picker path driven per-subscription via card-icon long-press (`input motionevent DOWN` 1.3s + `UP` — swipe long-press remains dead; NEW ops note: the list re-layouts between screencap and press, so capture + immediate press must be ONE command — two blind presses hit neighboring cards before this was fixed). **The three real lookups:** zohoaccounts.ca 14:05:52 `org="Zoho Canada Corporation" brand="zohoaccounts" corroborates=yes (org tokens match: zoho; From-host matches registered domain)` — org-token path, doubles as the C/D zohoaccounts RDAP half; zerosupporthosting.ca 14:57:17 `org="?" … corroborates=yes (no org-brand token overlap; From-host matches registered domain)` — registry answered, org withheld; dead-site chain correct: seeded TIER 0 → RDAP → TIER 0.5 `TypeError: Network request failed` (expected) → TIER 1/2 empty → TIER 3 Bing salvaged (UI: 3 valid saved, 2 retryable, 0 provider failures); x.ai seeded 16:33:30 → RDAP 16:34:51 `org="DNStination Inc." brand="xai" corroborates=yes (no org-brand token overlap; From-host matches registered domain)` — real answer behind a privacy proxy → TIER 0.5 `Extracted 12 icon URLs` → `Search completed for xai` 16:36:29 with filters visibly working (40 untrusted-provenance + 2 junk-farm + 3 brand-token-no-signal rejected; lobehub xai.png downloaded then `REJECT empty/transparent`; wrong-company xai.ma logo fetched then flagged Wrong by brand classification — G-class scope, recorded not fixed). **Negative case:** Proton picker search with empty TIER 0 discovery produced ZERO RDAP lines — RDAP strictly gated on discovery, by design. Screencap pipeline restored this session (valid PNGs `/tmp/pd_s13c.png`–`/tmp/pd_s20.png` after the earlier corrupt-capture issue). **Open carries:** zohoaccounts blue-mark icon spot-check (visual) still TODO; Phase E next (picker reveal-toggle fix, `SubscriptionIconPickerModal.tsx:848-863`). |
| 2026-09-12 | **PHASE D CLOSED — user-ordered ".io/.co seed hop" executed; .io/.co proven IMPOSSIBLE for RDAP at the registry level (external fact, not a bug); oddball met via .ai + .bot with a real on-device .bot registry answer.** User picked "add a .io/.co seed + rerun one picker search" after the substitution flag. Discovery chain: EMAIL-SEEDS scan has no .io/.co brand → candidate **wert → wert.io** verified live + brand-owned (wert.io operated by SHA2 Solutions, confirmed via site fetch) → device DB `files/SQLite/user_local.db` is ENCRYPTED (random-byte header, SecureStore present — direct sqlite seeding impossible) → implemented as curated code seed instead: `KNOWN_OFFICIAL_DOMAINS` map in `officialDomain.ts` + `knownOfficialDomainForBrand()` + TIER 0 branch (after session-seed/compound-slug, persists into `icon_crawl_sessions.official_domain`) — `1f339a0`. On-device picker rerun: seed fired (`TIER 0: Using known official domain for wert: https://wert.io`) but **`[RDAP] lookup wert.io unavailable (bootstrap/network)`** — root-caused to a REGISTRY FACT: rdap.org returns 404 "No RDAP service" for wert.io and `data.iana.org/rdap/dns.json` contains NO `io`/`co` entries (also missing us/me/so; ai/bot/app/dev present) — **.io/.co RDAP lookups are impossible for any client**; the resolver's unavailable-line is correct behavior. Oddball swapped to **cline → cline.bot** (Cline Bot Inc. card exists on device; cline.bot live + official; .bot RDAP answers via Nominet — verified host-side first). Slug-key fix (`clinebotinc` — keys normalize by stripping non-alphanumerics; first attempt keyed `cline` and missed) `40dbec2`; tsc/eslint clean, jest 286/286 + domain suites 4/4. **On-device result (Metro reload, no rebuild):** `TIER 0: Using known official domain for cline-bot-inc: https://cline.bot` → **`[RDAP] lookup cline.bot org="?" brand="cline-bot-inc" corroborates=no (no org-brand token overlap; no From-host known)`** — a real .bot registry answer; `corroborates=no` is the design working honestly (curated-seeded crawl has no From-host evidence; RDAP never auto-replaces uncorroborated) → TIER 0.5 `Extracted 24 icon URLs, queued 6 on official site`. Phase D totals: **4 real lookup domains** (zohoaccounts.ca org-token=yes; zerosupporthosting.ca From-host=yes; x.ai From-host=yes, .ai; cline.bot .bot, honest no) + negative case (no discovery → no RDAP). Ops: post-relaunch the self-heal chain re-runs and starves the JS thread — injected taps drop until the queue drains (wait for RN-quiet in the log); uiautomator dump + bounds-parsed taps beat coordinate guessing for the picker; picker X (uiautomator-verified bounds) still needed 2 taps + deep-search interference — left open at session end, cosmetic only. **Open carries:** zohoaccounts blue-mark icon spot-check (visual); Phase E next (picker reveal-toggle fix, `SubscriptionIconPickerModal.tsx:848-863`). |
| 2026-09-12 | **PHASE C (P3) CLOSED — Proton locked-scope SRP unlock now runs ONCE per scan instead of once per batch; live device gate PASSED with exact counts (unlock ok == 1, cached == 6, was 7/0 pre-P3).** Reauth-hardening hop from the original board, chosen by the user after the Phase-411 rundown. C1 finding that reshaped the fix: `unlockKeys`/`unlockLockedScope` live in `ProtonClient`, which is re-instantiated per `listWithSession` call — so the cache had to be FILE-SCOPE, not instance-level (`unlockedScopes` ConcurrentHashMap, uid → current accessToken, `d006703`). Guard at the top of `unlockLockedScope`; cache set only after the `users/unlock` PUT succeeds (failed SRP never poisons); token-refresh invalidation is free because the key contains the live accessToken — verified live: a P2 reactive refresh at 22:56:33 (`access token rejected — refreshing session`) fired BEFORE batching and the fresh token correctly re-keyed (first batch unlocked once, six skipped after). Per-batch key derivation left untouched (board specced the SRP skip only). C2 tsc 0 + jest 286/286 (native change has no jest coverage — device gate is the proof). C3 `build-android.sh --dev`: `BUILD SUCCESSFUL in 3m 58s` warm-daemon; trailing Metro skipped (8081 held by standing server, pid 708191). C4 gate: Settings → Clear Email Scan Cache (2218 → 0 via confirm dialog) → scan from Home. Proton leg 22:57–23:00: **`locked-scope unlock ok` == 1, `locked-scope unlock cached (P3)` == 6**, `Proton listed` 7/7 pages (75×6 + 31 = **475 of 475** — mailbox grew since the recipe's 451), every batch `Proton decrypted … fail=0`, `[MailProton-js] batches staged, messages=475 (chunk=75)`, zero PROTON_ABUSE/2028/429/CAPTCHA (grep noise = `max=429.34ms` frame stats + accessibility hex IDs), no reconnect prompt, Monthly Spend stable **$588.55** (recipe's $291.45 = stale calibration, superseded by later rescans); scan ended cleanly `scan ended (depth=0)` 23:05:49 (~14 min). Evidence `/tmp/p3_gate.log`. **Ops lessons:** (1) after rebuild+reinstall the app red-screens on `loadJSBundleFromAssets` — the adb reverse tunnel died with the rebuild; fix `adb reverse tcp:8081 tcp:8081` + force-stop/relaunch, then the app's own retry bundles from the standing Metro; (2) the emulator tab bar occupies y≈2114–2240 and silently eats injected taps — scroll targets fully clear of that band, RE-DUMP after every scroll (scroll settle moved the Clear Email Scan Cache button twice; four dead taps before the dialog finally appeared); (3) uiautomator can emit inverted bounds (`y1>y2`) for clipped nodes — normalize with min/max before tapping. **Open carries:** zohoaccounts blue-mark icon spot-check (visual); picker reveal-toggle fix (`SubscriptionIconPickerModal.tsx:848-863`); Monthly Spend audit ① ($588.55 vs $438.55 post-rescan delta); **next: Phase D (P4 official Proton client fingerprints) — last reauth-hardening hop.** |
| 2026-09-13 | **PHASE D (P4) CLOSED — Proton fingerprints landed as PARTIAL with the negative result conclusively proven: `x-pm-appversion` must stay `"Other"` (Proton ALLOWLISTS products — two server 2064 rejections prove third-party product names are impossible); UA hardened to stable `Cadence/1.0.0`. Reauth-hardening arc P1+P2+P3+P4 COMPLETE.** D1 premise correction: the research doc records NO literal official-web appversion/UA strings (§D5 documents only our old values), so the doc's own prescription governed (`cadence@<version>` + stable UA, no invention, no spoofing). Empirical chain, each step certified by the server: `cadence@1.0.0` → **HTTP 400 Code 2064** "platform and product must be separated by a dash" (`330f65c`); format fix `android-cadence@1.0.0` → **2064 "Product `cadence` is not valid"** (`e5cae32`) — the product field is allowlisted to official clients only; landing state `Other` + `Cadence/1.0.0` with the full evidence chain in the constants comment (`0e32c45`). Spoofing official client strings (e.g. `android-mail@…`) was considered and REJECTED as out of scope without an explicit user order — flagging it as the only remaining path if the user ever wants official-grade fingerprints. **D3 gate PASSED on the final state** (scan 05:32–05:5x, pid 3417): `Proton listed` **7/7** pages (75×6+31 = **475 of 475**), unlock **ok==1 + cached==6** (P3 intact through a fresh process), every batch `fail=0`, `batches staged, messages=475 (chunk=75)`, **zero** 2064/PROTON_ABUSE/2028/9001/429/CAPTCHA in the gate window (one grep hit = a SystemUI GC line), no reconnect prompt. The two 2064 rejections were themselves clean failures — typed `MailScanUnverifiedError`, leg failed gracefully, scan continued (P1 truthful-error path held under fire). tsc 0 + jest 286/286 at each step. Builds: 3m15s / 4m39s / 3m30s. Evidence: `/tmp/p4_gate.log` (both 2064 rejections), `/tmp/p4_gate2.log` (clean gate). **Ops lessons:** (1) an emulator that starts dropping/eating ALL injected taps (SystemUI ANR was the tell) needs `adb reboot` — post-reboot every tap landed first-try; (2) the reboot also kills detached logcat captures — restart the capture AFTER reboot or the scan evidence is lost (cost one scan); (3) the Settings cache-row Pressable spans `[0,1586][1080,2400]`, overlapping the tab bar — on Settings, ALL tab taps in that band are eaten (UI bug, recorded for the Phase-E UI hop); escape = force-stop + relaunch (fresh process opens on Home); (4) the Home scan CTA renders as a spinner (no text nodes) whenever the UI `scanning` flag is true — a missing button in dumps means CHECK THE FLAG, not "wrong coordinates". **Open carries:** zohoaccounts blue-mark icon spot-check (visual); picker reveal-toggle fix (`SubscriptionIconPickerModal.tsx:848-863`); Monthly Spend audit ① (now $588.55 → $438.55 → $188.21 across today's scans — needs its own verification hop); OkHttp/HTTP2 TLS fingerprint (optional, per doc); user decision available: spoof official client strings (rejected by default). |
| 2026-09-13 | **PICKER REVEAL-TOGGLES CLOSED FOR REAL (`7199efb` + `6f860c8`) — supersedes any earlier "hop 1 live-verified" claim, which was premature.** Live re-test on device found the reveal path still dead for session-reported icons: the `7199efb` refactor derived `visibleIcons` at render but left `submitReport`'s imperative `filter()` removal in place, so a freshly reported icon vanished from `availableIcons` and the toggles had nothing to reveal. Fix `6f860c8` marks `reportedType` in place. Full user-path gate PASSED on the embedded 09:45 build (linkedin, 2-icon collection): report → 1 visible; toggle ON → 2 + ✓ Good (zero reloads — `Loading icons for linkedin` log count unchanged across the toggle); ✓ Good → "Restored"; toggle OFF → 2 persist. Session also established (environmental, not app bugs): picker-open runs a ~44s collection heal that rewrites collections between opens (linkedin 4→2 icons; zoho/zerosupporthosting collections at 0 — "No valid icons to auto-assign" + 7-retryable is the dead-host drain signature); motionevent long-press injection died across the `adb reboot` (same-point `input swipe X Y X Y 900` long-press works — motionevent taps still fine); the subscriptions list resets its scroll on background data updates, so dump→locate→press must be one fast step after a settle. Residue: 3 QA test-report rows (2 linkedin orphans + 1 zoho) — inert byte-mismatches, test data. tsc 0 / eslint 0 / jest 286/286. NOTE: this row and the Phase E marker are the ONLY docs records of today's picker work — an earlier session's docs commit for the same scope never landed in this repo (reflog has no trace; IDE-crash era). Monthly Spend note from live reads: Home shows $438.55 stable across the day. |
| 2026-09-14 | **GOLD BASELINE `42a7435` + F-2 CLOSED (zohoaccounts → Zoho, Hops A/B/C + schema v16) + R13 / PHASE H CLOSED (`513245f`).** F-2: canonical-brand identity end-to-end — scans mint "Zoho", live card renamed, crawls seed zoho.com, real 77,608-byte tile preserved; device receipts in `docs/baselines/2026-09-14.md`. R13: per-leg progress pacer + 10-min scan-wide budget (between-legs check; watchdog keeps silence authority; `endScan` invariant untouched). Phase H gate PASSED live: steady-state scan **3m51s**, 4/4 legs with ≥1 `[MailScan] <leg> progress: N staged` line + terminal `leg done` line, zero budget lines, zero stalls, zero interventions (`[SCAN_STATE] scan ended (depth=0)` clean). One-time PARSER_VERSION-13 re-list measured **17m14s** (932 msgs restaged: proton 480, outlook 47, tuta 7, workspace 398) — observable line-by-line the whole way, self-terminating: the "runs without trickery" goal demonstrated on the worst-case scan. jest 302/302 (36 suites), tsc 0, eslint 0. **New findings opened, not yet fixed:** F-6 — `spend-projection-audit: projection=669.31 legacy=519.31 delta=150.00 match=NO` after scan #1's 59 workspace candidates; Home now shows $669.31 (projection path) vs pre-scan $599.68 (match=yes) — R26 dual audit caught a real disagreement, investigation owed. Gate A gap noted — a discovery flow started just before scan #2 (fastbtc) ran INTO the scan and starved its 3-message workspace leg for ~2.5 min (the drain pauses, in-flight discovery flows don't). Phase I (background scan UX) drafted in the phase plan per user directive. |
| 2026-09-14 | **J1 / F-6 CLOSED (`6fa33dd`) — the $150 spend disagreement was snapshot staleness, and Home now provably equals the row data.** Root cause: `useChargeDisplay` kept its boot-time `actuals` snapshot (581 pre-scan buckets) while scans rebuilt the projection (191 buckets) — the 16:26 audit compared fresh legacy rows (519.31) against stale buckets (669.31): `match=NO` with a PERFECT fold. Proof: cold-restart audit read both sides at 519.31, delta 0.00, `match=yes`. The spend drop 599.68→519.31 is the legitimate one-time PARSER_VERSION-13 re-classification (sparse −150.00, recurring +69.63) — both folds agree on the new rows. Fix: `onProjectionRebuilt(bucketCount)` fired by `rebuildProjectionAsync` (listener errors logged, never fatal) + the hook reloads `actuals` on every rebuild. Device gate PASSED on scan #3 (delta, 2m11s): rebuild→reload→audit `match=yes` mid-scan after every leg (17:21:36 / 17:21:47 / 17:21:58) AND post-scan (17:22:35, 17:22:42) with NO restart — the exact sequence that failed pre-fix; Home Monthly Spend $519.31 = both folds; 0-import dialog correct. jest 305/305 (36 suites), tsc 0, eslint 0. Next in Phase J: J2 (Gate A starvation). |
| 2026-09-14 | **J2 + J3 CLOSED (`3442fea`, `2fb1278`) — the scan now runs without trickery AND without blocking the user.** J2: discovery search parks at engine-stage boundaries during scans (`waitIfScanActive` injected as a stage gate) — paired-scan device repro PASSED (hangtag held through scan A; hflamtl's TIER-3 storm held through scan B, starting 274ms after `scan ended`; legs fast, zero stalls, nothing dropped). J3 (Phase I I1–I3): `scanProgress.ts` external store wired into the leg loop + `useScanProgress`; Home scan button is fire-and-forget — live `Scanning… leg k/N` pill from the store (survives navigation; observed leg 1/4 → 4/4 across a Home→Subscriptions→Home round-trip mid-scan), success completion is a toast (system Toast-window receipt), R2 errors keep the Alert. Device gate PASSED end-to-end (scan 18:36:49→18:39:21; post-scan audit `match=yes`, Home $519.31). jest 313/313 (38 suites), tsc 0, eslint 0. Remaining in Phase J: J5 (dead-host strategy — spec locked), J4 (hygiene sweep). I4 icons-during-scan stays OOM-gated. |
| 2026-09-14 | **J5 CLOSED — dead-host popup + short-circuit proven live on device (impl `fd8ee76`, docs-only close; zero code changes this session).** Demo executed with a REVISED protocol after the original one failed on a discovered protocol defect: post-cache-clear full re-scan ran clean (934 classified rows re-created, spend 519.31 restored) but zerosupporthosting never re-imported — `[MailGmail] limit 500 reached — truncating listing at 500 ids (more pages existed)`: the `57bc9ab` INITIAL_SCAN_LIMIT cap now bounds the Gmail page walk, so a deleted brand whose mail ages past the newest 500 ids can never re-import by scan alone (PARSER-13's 17m14s/932-msg re-list predates the cap). **Watch item opened: cache-clear re-scan under the 500-id cap loses old classified brands permanently — a future backfill tranche may owe a one-time deep re-list.** Revised demo (manual add → crawl → probe → popup → Keep → short-circuit): CreateSubscriptionModal row "Zerosupporthosting" $1.00 → `startIconCrawl` (sub 1789432776487, 20:39:36); stored crawl rows seeded TIER 0 (`Using seeded official site: https://zerosupporthosting.ca`); 0 downloads → **20:53:58 `host liveness: zerosupporthosting.ca = unregistered (score 95)`** (RDAP 404) → popup LIVE (/tmp/j5_POPUP.png): "domain unregistered — registry: domain not registered (RDAP 404); DNS: NXDOMAIN; A=no AAAA=no MX=no NS=no; all crawl fetches to this host failed" with KEEP / MARK AS CANCELED / DELETE; KEEP tapped → card retained, decision persisted; re-crawl via picker "Search for Icon Online" → **20:57:09 `short-circuit: zerosupporthosting.ca is a known dead host (cached liveness ≥85) — skipping web discovery`** in 614ms with zero network. Test row deleted after (subs 124, 519.31 restored). Ops: the dedicated `timeout 3000` logcat capture stalled mid-run and dropped a JS-line window — the standing `/tmp/scan_logcat.log` carried the evidence. Receipts in `docs/baselines/2026-09-14.md`. Remaining in Phase J: **J4 hygiene sweep (last item)**. |
| 2026-09-14 | **J4 CLOSED — verification & hygiene sweep complete; PHASE J COMPLETE (docs-only close; zero code changes this session).** 6-step reveal-toggle protocol re-run on current HEAD (post F-2 rename + J5 crawl changes) on the Zoho picker, every step uiautomator-driven: Wrong+comment → "Icon Reported"; count 2→1 (wrong-reported tile hidden, toggle off); "Show incorrect" ON → 2 again with NO reload (`Loading icons for zoho` count 20→20 across the flip — the derived-filter assertion behind `6f860c8`); ✓ Good chip IN PLACE on the reported tile → "Restored" alert; count stays 2; toggle off → stays 2 (state-neutral; persisted state proven by a fresh 3rd open: 2 icons, no ✓ Good). G1 brand-grid gate re-run: 4 pickers — zoho 2/2 brand-correct (both F-2 tiles), microsoft 5, google 5, linode 9 linode-dominant; zero PNG-farm junk anywhere; J5-probed brands had 0-download crawls (no grid change possible). Baseline row 15 reconciled: the protocol had ALREADY run 09-13 (LESSONS 37 caught the `submitReport` residue → fixed `6f860c8`); only the HEAD re-run remained. zohoaccounts "flagged for D/E" closed as MOOTED by F-2 (identity was the defect; zoho grid 2/2 brand-correct today; Phase C row annotated). Opportunistic email-seed apply protocol documented at the C4 gate spec (trigger, picker-fired single-brand crawl, ordered witnesses, PASS = ≥1 brand-correct email-sourced apply; zerosupporthosting permanently excluded — dead per user, J5-short-circuited). Device state unchanged: subs=124, Home $519.31, no popup (`/tmp/j4_16_final_real_app.png`). Ops: (1) report-dialog buttons sit under the soft-keyboard window — dismiss the keyboard before Submit (attempt 1 silently hit the keyboard); (2) a BACK-exit + Expo Go relaunch (wrong container) showed an auth gate + subs=0/$0.00 from Expo Go's own fresh sandbox — real app data untouched throughout (dev-build DB 53,809,152 B, mtime 21:30, verified read-only via adb root); remedy = always launch `app.picksandshovels.cadence`; (3) `adb root` killed the standing logcat capture — retired deliberately, `/tmp/scan_logcat.log` kept as artifact. Receipts in `docs/baselines/2026-09-14.md` (J4 section). Remaining in Phase J: **none**. |

---

## Phase K — deep re-list with progress bubble (user-directed 2026-09-14, COMPLETE — gate PASS 2026-09-15)

**User story:** after Clear Email Scan Cache (or on demand), the user can re-list their ENTIRE mailbox history — not just the newest 500 ids — so brands whose mail aged past the `INITIAL_SCAN_LIMIT` window re-import (the J5 watch item). A warn modal states the cost; a dismissible floating bubble with a circular gauge shows live progress ("X of Y emails scanned"), tappable for a status panel; per-mailbox scan errors surface there (retires the I3 residual's UI half).

- **K1 engine:** `deep` mode threaded through `importFromConnectedMailboxes → provider.scan → runPersistedScan → runIncrementalScan`: `since=null` (ignore the cursor; cached messages at the current `PARSER_VERSION` still skip, so a warm-cache deep run only stages what is missing), `limit=DEEP_SCAN_LIMIT` (50,000 — a runaway ceiling, not a work bound), scan-wide budget `DEEP_SCAN_BUDGET_MS` (60 min, between-legs check unchanged), R14 watchdog + R12 pacing untouched. Witness: `[MailScan] deep re-list: …`.
- **K2 totals:** `ChunkMeta {listed, total}` streams from every fetcher's `onChunk`; Gmail `resultSizeEstimate`; Graph `$count=true` → `@odata.count`; JMAP `calculateTotal` (best-effort); Zoho/Proton/Tuta-native/IMAP count-only (`total: null` — honest unknown). `scanProgress` store gains `{deep, listed, listedTotal}` with cross-leg roll-up; UI renders "scanned N" when no total exists (never a fake percentage).
- **K3 UI:** Settings → **"Deep Re-list (entire mailbox history)"** (accent button, warn bottom sheet: cost + what it fixes) → same fire-and-forget + toast/alert completion pattern as the J3 Home handler; **post-cache-clear offer sheet** ("Re-list your entire mailbox history now?"); **`DeepScanBubble`** mounted in `app/_layout.tsx` above `<Stack>` (pointerEvents box-none): svg ring gauge, center = % when a total is known / compact count otherwise (exact "X / Y emails scanned" in the expanded panel — 64dp legibility tradeoff, documented), tap → panel (counts, leg k/N, mailbox, staged, imported, errors, Hide). Deep-only v1; normal scans keep the J3 pill.
- **K4 unit:** deep passthrough (since=null, deep limit, warm-cache skip), non-deep regression guard, budget deep variant, scanProgress totals roll-up incl. unknown-total legs + failed-leg rollup, Gmail meta capture (`listed` counts screened ids; `resultSizeEstimate` → total). tsc/eslint/jest green.
- **K5 gate (device):** Clear Email Scan Cache → accept the offer → deep re-list runs with the bubble visible at two captured moments; ≥1 brand aged past the 500-window re-imports (zerosupporthosting is the natural target — its mail is old and its Keep decision is persisted, so no popup spam; the dead-host short-circuit makes its crawl cheap); subs/spend state sane afterwards; a normal Home scan still shows the J3 pill (regression).
- **Excluded v1 (user-agreed):** mid-scan Stop (the 60-min budget bounds it); bubble for normal scans; per-fetch TIER-3 gating (residual recorded in J-era docs — the per-fetch `waitIfScanActive()` idea for the immediate-batch + TIER-2 loops is queued as a cheap follow-up).
- **K5 gate result (device, 2026-09-15) — PASS.** Fresh-install state ($519.31 / subs=124) → warn sheet (copy + maxHeight fix after k06 caught Cancel clipped off-screen) → Clear Email Scan Cache (934 msgs) → offer sheet → **"Deep re-list now"** at 00:15:46. Run: `[MailGmail] listed 2098 pages=21 bodies=1695 of 2098` (workspace leg alone; **zero** `limit 500 reached` lines in the whole run), legs tuta 7 / proton 483 / outlook 47 / workspace 1695 staged; `scan ended (depth=0)` at 00:57:45 = **42 min**, inside the 60-min deep budget (the old 10-min budget would have killed it mid-workspace). **zerosupporthosting re-imported**: subs 124→**125** (`home render subs=125`, new row id 1789448262985), its crawl fired at 00:57:45 the instant the scan ended (Gate A order) and the J5 dead-host short-circuit handled it (~7s); Keep decision suppressed the import popup. Spend integrity: mid-scan audits all match=yes; the 00:58:52 audit read `match=NO delta=150.00` during the documented R26 convoy window (post-workspace rebuild queued 134s behind the drain crawls) and self-reconciled at 01:00:53 `match=yes 599.68/599.68`; Home renders **$599.68** with the Zerosupporthosting $1.99 card present (icon stays "+" — dead host, Phase O owns no-icon placeholders). Bubble UX: seen in count-only mode at start (k09, "0") and 100% green-ring mode once Graph reported its total (k11/k10); deep button disabled while active, re-enabled after. **Regression**: a normal Home scan right after shows the J3 pill `Scanning… leg 1/4` (a11y + k16 screenshot), no bubble node, completes in ~95s with all legs `0 staged` (warm-cache skip — nothing left to re-stage), same candidate counts. Receipts: `/tmp/k_gate.log`, `/tmp/k09_bubble_early.png`, `/tmp/k10_panel.png`, `/tmp/k11_state.png`, `/tmp/k13_home_top.png`, `/tmp/k15_top.png`, `/tmp/k16_pill.png`, `docs/baselines/2026-09-15.md`.
...
- **O3 DEVICE GATE — PARTIAL (commits `51a7d4c`, `87700e5`, `673e761`)**:
  ...
  - **Share-sheet gap (environment)**: bare emulator has no share target ("No apps can perform this action") — file pulled via run-as for the gate; on real devices the sheet works (mimeType application/octet-stream).
  - **Phase A REGRESSION FOUND + FIXED (`673e761`)**: fresh install on a lock-screen device deadlocked (resolveGate device-prompt + noCreate open → first key never minted → "Vault is locked / Database Error" loop). Fix: mint the first key after OS verification IFF no DB file exists for the identity.
  - **DATA LOSS (user must decide next steps)**: the clean-room gate wiped the dev device (pm clear) while the only backup was an INTERMEDIATE-format file whose header-write corrupted page 1 (the ATTACH-pragma plaintext-header reservation did not apply; 24 offline sqlcipher parameter combos tried — all hmac-fail). The 136-sub dataset, mailbox sessions, and scan cache on the dev device are UNRECOVERABLE. Recovery path per product design: re-connect mailboxes and re-scan on the fresh install.
- **NEXT for O3 gate**: rebuild data (re-scan or manual test rows) → export → pm clear → fresh install → import → verify subs + spend match. Then docs/LESSONS updates.
- **O3 GATE — PASS (session 2026-09-16 afternoon, commits `ed37ea9`, `2ac16f0`)**:
  - **Export SIGSEGV root-caused + fixed (`ed37ea9`)**: both Export taps killed the process natively — `exportBackup()`'s closeDatabase() close-window raced the background crawl-heal, use-after-free in libexpo-sqlite (`sqlite3_column_name`). Fix: the encrypted export ATTACHes the passphrase-keyed `.cbak` to the LIVE connection and runs `sqlcipher_export('exp')` in place; no closeDatabase on the path. Proven: export ran with the heal loop alive, app survived, 7.38MB file written.
  - **Backup fidelity fixed (`2ac16f0`)**: the export's strip list (borrowed from cloud-sync scope) deleted `mail_messages` — the classified corpus that sparse spend folds from ($74.84 of $355.79) — and the import restored only `subscriptions` (Home rendered $0.00, buckets gone). Fix: encrypted export keeps `mail_messages` (crawl ephemera + `mail_mailboxes` OAuth identities stay stripped — tokens are install-local); import restores `mail_messages` + `merchant_day_actuals` buckets (0-row no-op for cloud-sync payloads).
  - **GATE RESULT**: pm clear → fresh install → import gate.cbak → "53 subscriptions imported", Home **$355.79** (/tmp/gate_home.png). Audit honestly reads `projection=355.79 legacy=280.95 match=NO` — the legacy fold is low ONLY because gate.cbak was produced by the pre-`2ac16f0` export that stripped the corpus; post-`2ac16f0` exports carry it (next full-fidelity backup comes from the fixed export after a scan).
  - **Remaining O3 items**: reconnect both mailboxes on the clean install (by-design absent; consent flow proven repeatable), then one scan + re-export to produce the first full-fidelity backup and re-verify the audit reads `match=yes` at the same number end-to-end.
- **O3 FULL-FIDELITY GATE — PASS (session 2026-09-16 evening)** — O3 device gate CLOSED:
  - Gmail reconnected on the clean install (OAuth "Unmatched Route" flash is COSMETIC — token exchange completes); rescan rebuilt the corpus (400 classified rows) → audit `355.79/355.79 match=yes`. Workspace mailbox left disconnected BY CHOICE: contributes $0 to spend, carries the known 180s stall (tracked as its own issue).
  - First full-fidelity export (18.8MB) verified OFFLINE with sqlcipher3: `mail_messages` 400 rows IN the file + buckets 236 + subs 53 — the discriminating proof of `2ac16f0`'s export half.
  - pm clear → fresh install → import → audit `projection=355.79 legacy=355.79 delta=0.00 match=yes`, Home **$355.79** (/tmp/gate3_final_home.png). Both halves of `2ac16f0` are now device-proven end-to-end.
  - Recorded behavior: mailbox connect RESTAGES buckets (derived cache) — a corpus-less import reads subs-only until rescan (LESSONS 40). A copy of the full-fidelity backup is preserved at /tmp/gate3_full.cbak.
  - Cleaned: /sdcard/Download + cache/exports/.
- **Workspace "180s stall" — ROOT-CAUSED + CLOSED (2026-09-16 evening, no code change needed):**
  - The "stall" is the R14 no-progress watchdog working as designed: `NO_PROGRESS_BUDGET_MS = 180_000` (scanWatchdog.ts) fails a leg after 180s of silence. The stall_hunt capture caught the pre-wipe smoking gun in the logcat ring buffer: 13:11:33 `armed 180000ms` → **13:14:33 `no scan progress for 180000ms — failing the leg`** — exactly 180s, on the old install.
  - The silence itself is the 2026-09-05 wedge class (mid-scan network transients parking the JS fetch loop) — environmental, with mitigations already layered (F-4 per-chunk feeding, R16/R17 token margins, quota backoff feed, R13 total budget).
  - Live verification on current code: reconnect Workspace → full scan → the leg ran **8.5 min without a single silent 180s** (continuous page/screening progress), completed `400 staged, 61 candidate(s)`, watchdog armed but never fired, audits `match=yes` throughout. Workspace mail imported real subscriptions (subs 53→55; spend peak `$711.58`, audit `711.58/711.58 match=yes` during the drain).
  - Failure loudness confirmed in source (LESSONS 26 amendment): a watchdog-failed leg now reaches logcat (`leg failed` warn), the scan-progress pill, and the end-of-scan errors list. Any future recurrence will be visible AND bounded; reopen this item only with a fresh capture showing repeated watchdog fires on healthy network.

### Queued after K (user-directed order: K → M → L → N → O)
- **M — triage honesty (the "Gmail $1,439" phantom, user report 2026-09-14):** freemail-host map (gmail.com/googlemail.com/outlook.com/hotmail.com/yahoo.com/icloud.com can NEVER mint the merchant from the host alone); forward detection (subject `fwd:`/`fw:` markers + the "Forwarded message" header block — extraction at classification time where full HTML still exists) → merchant evidence from the ORIGINAL issuer's From-host inside the forward, else DROP the message (early-project spec: not every email is a subscription); non-forwarded freemail senders (small businesses billing from gmail) fall back to display-name/body evidence and import sparse; canonical Google-product map so google.com-family senders resolve to the right product (Cloud/Workspace/One/YouTube/Play) without conflating into "Gmail". `PARSER_VERSION` 14. The existing bogus row needs a one-time manual delete (no migration invents data). Unit fixtures incl. the friend-forwarded-bill case. **DONE 2026-09-15 (`268d743`) — Gate PASSED.** Implementation: `FREEMAIL_HOSTS` + `resolveFreemailSender` + `resolveGoogleSender` + `GOOGLE_PRODUCT_MATCHERS` (Workspace/GSuite, Cloud/GCP, Ads/AdSense, One, Play, Drive, YouTube → product host as officialDomain, generic google.com sender → "Google", never a mailbox label); forward path scans the RAW text for the quoted `From:` line (moneyBodyText strips angle-bracket addresses as markup — raw-first was required, hit live by the fixture) with stripped-HTML second chance; original issuer google.com-family → re-keyed to the product; **drop rule refined**: unresolvable forward DROPS only when a body "Forwarded message" marker proves it — a subject-only `Fwd:` falls back to display-name/body sparse (dropping would have lost the real Raceroots row, see gate); non-forwarded freemail → display-name (angled, non-@) then body evidence, `forceSparse` kind even for recurring-looking subjects; bare freemail address mints nothing. `PARSER_VERSION` 13→14 (all v13 caches restage on next scan; body-derived re-keys refresh on live re-fetch — stripBodyForStore comment updated). Fixtures `FIXTURE_FRIEND_FORWARD` + `FIXTURE_FREEMAIL_BUSINESS` in ALL_FIXTURES/index exports; 9 new classifier tests. jest **346/346** (41 suites), tsc 0, eslint 0. **Device gate (IDE crash killed the emulator; relaunched pixel_6a_API34 cold, subs=125 invariant held):** phantom found via search — "Gmail, sparse, $250.00 Monthly", Details showed Source `Sean <seantorjman@gmail.com>` · "Fwd: Order confirmed: MAZDA Genuine RX-8 …" (the exact fixture shape, real mail); one-time manual delete via card ••• → Delete → confirm (search then `Results (0)`); **deep re-list** from Settings ran 10:45:39→11:28, legs tuta 7 / proton 484 / outlook 47 / workspace **1695 staged, 134 candidates** (=2233 msgs restaged under v14), audit `match=yes` (spend 449.68→**644.72** = legitimate aged-out re-imports, e.g. 420stock $175); **bill 100001140 now imports as "Raceroots - Sales — sparse — ?"** (source `raceroots - sales <raceroots@gmail.com>` — the seller itself bills from freemail, so forward→inner-freemail-skip→display-name sparse fired on real mail; brand-correct, no invented amount, NO "Gmail" row), post-deep gmail search `Results (0)`, follow-up warm scan all legs `0 staged`, audit `match=yes 644.72/644.72`. Receipts: `/tmp/m_gate.log`, `/tmp/m01_menu.png`–`/tmp/m09_subs.png`, `docs/baselines/2026-09-15.md`.
- **L — email-icon immediate populate:** `tryApplyEmailIconDirect` (fetch ≤3 best-first, validate + report-stick, save with `email_logo` provenance, promote) tried in `enqueueScanIconCrawl` BEFORE `startIconCrawl`; success skips the web crawl entirely; failure falls back to the seeded crawl (today's behavior). Gate A respected: the chain parks on `waitIfScanActive()`, so the apply lands at drain — the moment the scan ends, ahead of every crawl.
- **N — subscription status UX:** "may have expired" chip (now > next-renewal + grace), grace-threshold control in Settings, filter chips Active/Sparse/Expired/All on Subscriptions (chips already exist — extend `FILTER_OPTIONS`).
- **O — UI polish + safety net:** no-icon placeholder (replace the `plus.png` card fallback with a muted no-thumbnail glyph — `SubscriptionCard.tsx:103-105`), `MAIL_PROVIDER_CATALOG` brand icons on the Add-mailbox rows, and the export → wipe app → fresh install → import clean-room device gate (the SQLCipher escape hatch is UNPROVEN; R25-b proved raw-db file backups are unrestorable).
- **A — app auth & local key custody (user-directed 2026-09-15: Auth BEFORE L2):** the app never opens without at least one factor; the OS decides which. Factor gate at cold open in `DatabaseProvider` before `openDatabase`: `expo-local-authentication.getEnrolledLevelAsync()` → `NONE` (no screen lock) = foot-down, force app-pass creation (A3) before the DB is ever created; `SECRET`/`BIOMETRIC` = device prompt (`authenticateAsync()` — biometry if enrolled, else the phone's own PIN/pattern/password; NEVER force biometry), once per cold start, in-memory unlock flag. Custody stays per-user random 256-bit in SecureStore (Keystore-wrapped; `LOCAL_USER_ID="local"` namespace already exists in `AuthContext.tsx` — backend ids must never collide with it). **A3 app-pass leg** (forced for NONE, optional switch in Settings for others): wrap the random DB passphrase — `blob = AES-256-GCM(KDF(appPass), dbKey)` — store blob + salt + iterations + version, DELETE the SecureStore plaintext; change pass = unwrap-with-old → re-wrap; KDF = PBKDF2-HMAC-SHA256 via a small in-repo Kotlin module (precedent: `cadence/` native modules; expo-crypto SDK 54 has NO pbkdf2 — verified) → one dev-client rebuild. **A4 recovery phrase:** 128-bit entropy → 12-word BIP-39 English list (checksum; ~13KB wordlist in JS, pure functions) → second wrapped copy `blob_recovery = AES-GCM(KDF(phrase), dbKey)`; shown ONCE at app-pass creation: full-screen, FLAG_SECURE window (no screenshots, no recents thumbnail), non-selectable render, never persisted/logged; copy = memory-first ("try to commit them to memory; if you must write them down — paper, kept secure; order and spelling matter"), screenshot sidenote ("capturing any other way is at your own risk"), then "I have saved these words" checkbox + verify-back (3 random words); settings offers regenerate (re-wrap, invalidates old) but never re-display. Recovery flow: enter 12 words → unwrap → force new app pass → re-wrap both blobs. Forgotten pass with no phrase = data gone → rescan as new device (stated plainly in UX). A1 resolved as already-built: `AuthContext` local session IS the local-only mode. Device gate (A5): fresh-install walkthrough (forced creation → phrase screen with screenshot-block verified → relaunch prompts → full recovery via phrase), spend audit `match=yes` after a real scan.
- **L2 — cid inline-part plumbing: CANCELLED BY EVIDENCE (probe scan 2026-09-15, v17).** Step-0 probe answers: of 116 classified candidates (tuta 2, outlook 3, proton 9, workspace 61+, rest warm), **0 carry cid refs** (total cidRefs=0; proton chunk probes: every 75-msg chunk `withContentID=0 withImagePart=0 withCidRef=0` — Proton decrypted bodies carry no inline image parts at all, so Branch A has nothing to extract and Branch B has nothing to fetch); meanwhile **44 candidates yield 129 absolute-URL seeds** (Cline/Stripe 26, OpenRouter 15, Porkbun 3) — the Phase C extractor already surfaces them. The real L residual is the **enqueue gate**: `scanConnected` fires `enqueueScanIconCrawl` (+ EMAIL-DIRECT) only when the existing row's `icon_key` is missing or "plus" — by design (crawl owns publication for already-icon'd brands), so with all 136 subs icon'd, seeds never reach the crawl. Demo/decision options for the user: (a) keep design — the gate fires naturally when a NEW icon-less candidate with seeds imports (e.g. a fresh seed-bearing invoice); (b) extend the L chain to also fire for icon'd subs (new promotion-semantics decision, user call); (c) live demo by clearing one seeded brand's icon (picker) and rescanning. Probe instrumentation removed post-scan; PARSER_VERSION retained at 17 (reverting would force a redundant restage).
- **User confirmations recorded 2026-09-14:** SQLCipher is by-design (privacy); deep re-list triggers = BOTH the Settings button and the post-cache-clear offer; phase order K → M → L → N → O.
- **NEXT SESSION QUEUE (user-agreed 2026-09-15, after L2 probe cancelled the cid tranche):** (1) **Curated provider-brand map** — branded MAIL_PROVIDER_CATALOG rows gain canonical icon identities; slug-set mapping (tuta, tutanota, proton, gmail, outlook, zoho, fastmail, …) resolves supported-provider cards to the curated icon by default, first place; crawl/acquired icons rank behind; manual pick still overrides. Fixes the Tuta wrong-brand-first card (Tuta Group ≠ Tuta the email company). (2) **R18 billNumber pattern for Tuta invoices** — the Tuta row exists (sparse, ? price, correct) but billNumber extraction found nothing; add Tuta invoice-number format to the R18 patterns. (3) **Sparse-spend clean investigation** — the 572.56→311.09 sparse drop (recurring stable) needs a clean logcat capture around a single scan cycle; the $47.74 reading was a misread of Porkbun's price in search results (retracted). Then N/O phases as queued.- **User confirmations recorded 2026-09-15:** Auth phase (device-credential default, never force biometry, no-screen-lock users foot-downed into an app pass) ships BEFORE L2; recovery-phrase approach approved with memory-first copy and screenshot-block as best-effort ("if they screenshot after the warning, it's on them" — warning carries it); Clerk fully dropped — `AuthContext` local session is the local-only identity (`user_local.db` naming on the dev device predates this).





## R38 — Truth-first parsing: the six named rows (user-approved 2026-09-18)

**Named outcome (the ONLY finish line):** the six-row before/after table, produced on device after a restage scan under the new PARSER version. A green build, a clean Insights list, or a restored backup is NOT done.

| Row | Today (restored corpus) | Must become | Fix phase |
| --- | --- | --- | --- |
| Hightimes | $59.99 · recurring Monthly | gone-or-sparse (it's ads) | P3 ad gate |
| RockAuto | sparse $0.00 "This month" (data is IN the email HTML) | real amount + bill no. + payment method | P1 (DONE `fe9ff68`) |
| Squarespace | "Squarespace" merchant row ($11.99/Monthly) | resolved to the real service, or dropped (processor) | P2 rail policy |
| Netgate | recurring, "? Monthly/Yearly" | **free $0** (one-time license) | P3 $0 flip |
| RotaryEngine | sparse $0 Monthly (legacy stamp) | honest window, no fake Monthly stamp | P3 un-stamp guard |
| Google Play receipts | keyed to rail "Google Play" | keyed per receipt item/app (rail = context) | P2 per-item keying |

Google One $26.99: **settled by user 2026-09-17 — legit recurring, untouched.**

**Execution order (approved):** P0 audit → P2 → P3 → THE GATE → P4 → R39.

- **P0 — DONE (2026-09-18, device audit, "before" column locked):** six reported rows audited on device via card ⋮ → Details (Edit sheet peeked for DB ground truth where the card render was suspect); screenshots `/tmp/p0_*.png`. Findings per merchant:
  - **Hightimes** — $59.99 recurring Monthly "May have expired" · `workspace:david@bohbotweb.com` · Started 09/16/2026 · Bill — · Source email —.
  - **RockAuto ×2** — $74.84 sparse "This month" + $80.96 sparse fake-Monthly twin (R37-era mint) · both Source email —.
  - **Squarespace ×2** — $41.00 Monthly recurring (workspace, "May have expired") + $41.00 Monthly recurring (gmail). SAME plan on two mailbox identities — R37 never claims cross-mailbox, so both persist. BUG: the gmail twin's card intermittently renders **$492.00 Yearly** (41×12) while its own Details + Edit sheet prove Price 41 / Monthly.
  - **Netgate ×2** — $0.00 Monthly recurring (workspace, "May have expired") + $0.00 Monthly recurring (gmail) — the $0-license class P3 flips to free.
  - **Rotaryengine ×2** — $0.00 sparse (workspace, "This month") + $0.00 sparse Monthly (gmail).
  - **Google Play ×2** — $68.97 Monthly recurring (workspace, card stacks "+$67.96 Sparse") + $68.97 Monthly recurring (gmail) — the rail minted as merchant; P2's per-receipt-item keying target.
  - **UNIVERSAL:** every audited row shows Source email —, Bill number —, Started 09/16/2026 — one scan minted them all; the restored corpus rows carry no UI-visible paper trail, so the "before" column rests on row state + corpus mail evidence, not per-row email links.
  - **UI bugs recorded (parked as UI ticket / R39 candidates — explicitly OUT of P2/P3 parser hops):** (1) duplicate-name cards share React state — tapping twin card 2's ⋮ expands/opens card 1's menu and Details can open the twin's sheet (same name-keyed class as `ee4f9d7`); (2) the search keyboard overlays and hides the second results row — list is not keyboard-aware (user-reported 2026-09-18; dismissed keyboard required before multi-row captures); (3) the Squarespace gmail twin's Yearly render flip above.
- **P0.5 — DONE (2026-09-19, scan-date fix `df7b98f` + `0229410`; user-reported during P0 review):** "Started" used the scan wall-clock, not the email received date — bug present since the original emailscan commit `2dde162` (2026-08-18): `candidateToSubscription` stamped `startDate: new Date()`, `ScanCandidate` carried no date at all, and no repair gate ever touched `startDate`. Fix: rollup derives `firstSeen` (min corpus `message.date`) → mint uses it (wall-clock only as guarded fallback) → `startDateRepair` heals stored scan-day stamps MIN-ONLY (never moves a start later; price/cadence gates untouched). Second widen (`0229410`, user directive): the heal applies to EVERY matched row — hand-entered and restore-era rows included — because Started is evidence-derived, while the R18 sparse-never-repairs-recurring rule keeps guarding price/cadence. NO parser bump (rollup re-derives from stored classified rows). Tests: `startDate.test.ts` 11 cases; jest 494/494, tsc+eslint clean. Device gate: deep re-list (both legs, history exhausted) → YouTube row Started **01/07/2023** (earliest corpus email), Hightimes twins 09/16/2026 = their true earliest evidence (no older mail exists — deep walk exhausted at page 22/+2 ids). Corpus now 207 subs; deep-restore legitimately re-imported ~104 aged-out subscriptions; Monthly Spend re-baselined ($484.75 → $380.18 after heals/lapses). The P0 "before" column (all rows Started 09/16/2026) is thereby explained and the six-row after-column will be read under this fix. Device unlock pass + backup passphrase are recorded in the Session context block below.
- **P0.6 — DONE (2026-09-19, sentinel recovery — dual-leg twin purge):** the agent connected the ONE physical mailbox david@bohbotweb.com as TWO provider legs (workspace + gmail) against the user's explicit instruction, then deep-scanned both — every merchant minted twins (subs 103→207; the pre-existing P0 twin pattern and the 09-17 "Amazon payment_method-mismatch" backlog item share this root). Recovery on device: emulator cold-booted from surviving userdata (921MB qcow2) after a crash; checkpoint pulled (~/cadence_checkpoints/user_local_20260919_021456.db); Gmail leg removed via the app's own path (Remove → "Remove subscriptions too": 103 gmail-keyed rows deleted + leg mail_messages purged + disconnected); clean workspace-only scan re-imported exactly 2 (no twin resurrection — purge verified by design); final `spend-projection-audit: projection=148.83 legacy=148.83 delta=0.00 match=yes`, subs=106, ONE mailbox. **Six-row Started verification (after-column, receipts ~/cadence_checkpoints/six_row_receipts/):** Hightimes 09/16/2026 (true earliest evidence — deep walk exhausted history), Squarespace **12/29/2024**, RockAuto **Jan 25 2026**, Netgate **Feb 2 2026**, Rotaryengine **Jan 10 2025**, Google Play **Nov 25 2022** (+cadence healed Monthly→Yearly honestly). Every Started is now the earliest RECEIVED email date. Correction to P0.5 above: the "+104 aged-out subscriptions" claim was made before the twin purge — of the 207, 103 were gmail-leg duplicates; the honest single-leg world is 106 rows.
- **P1 — DONE (`fe9ff68`, PARSER v27):** ld+json Order markup = top evidence tier; price from nested `acceptedOffer.price`; `orderNumber`|`confirmationNumber`; seller/items/order-no carried downstream for P2. Unit-locked in `orderMarkup.test.ts`. TRAP: markup evidence tags (`markup:order`, `proof:order-markup`) are classifier-level — logcat only, never a Details field.
- **P2 — Billing-rail policy (PARSER v28):** (1) Play/Apple are rails, never merchants — classify PER RECEIPT ITEM via P1 `orderMarkup.items` + real corpus Play receipts (web help pages 404; corpus is ground truth): recurring item → keyed by app (e.g. `tinder`), one-time IAP → sparse keyed by app, unresolvable → honest aggregate "Google Play" sparse. (2) Squarespace → `PAYMENT_PROCESSORS` (classifier.ts ~L82) + receipt site/service extraction; unresolvable Squarespace mail DROPS, never mints. Unit fixtures per case; tsc/jest clean; commit.
- **P3 — Ad gate + one-time fixes:** (1) marketing/offer subjects never mint RECURRING without a proven payment; Order markup with real price/orderStatus counts as proof (extends R27 purchase-proof gate). (2) $0 license receipts: cadence anchors ("renewal price", "domain registration") never fire; recurring+$0 flips to free (scanConnected stream-flip already accepts recurring→free; classifier must stop anchoring). (3) ONE proven-payment message with no cadence words clears a legacy stamp (loosens R34 ≥2-message `cadenceClear`; hand-entered rows still protected; never recurring rows). PARSER bump; tests per class; commit.
- **THE GATE (R38 acceptance):** agent re-auths both mailboxes on-device from `.env` creds (GMAIL/WORKSPACE_USER+PASS; user pinged ONLY for 2FA/captcha — never frame re-auth as the user's task) → restage scan under new PARSER → before/after table for all six rows. Regression guards in the same run: no twin re-mints (R37 classes stay dead), subs count moves only by intended flips, spend audit `match=yes`. Docs/baselines + this plan updated only after the table passes.
- **P2 — DONE (`f39a9c1`, PARSER v28):** rails never mint in their own name — Play/Apple receipts re-key per receipt item (`railItemFromReceipt`: P1 `orderMarkup.items` top tier, subject/body fallback; first-party keeps the rail name; unresolvable/multi-item → honest rail-keyed SPARSE aggregate); Squarespace → `PAYMENT_PROCESSORS` + site/domain extraction (`squarespaceSiteFromText`) or drop. 10 tests.
- **P3 — DONE (`181cc0b`, PARSER v29):** marketing-shaped recurring demoted without strong proof; `$0` license receipts never anchor cadence (`zeroLicense` suppression); `orderStatus` counts as strong proof; ONE proven-payment message clears a legacy sparse stamp (R34 loosened, hand-entered rows protected); `$0→free` flip carries the stamp clear. 6 tests. Flip fix `886c8e6`: `kindCompatible` admits free candidates; `richer` guarded (free never rewrites price). 1 test.
- **Audit fixes — DONE (`2ebd7db`, PARSER v30, 2026-09-19; granular self-audit found 4 live defect paths in my own P2/P3, none yet fired on the corpus):** (A) free-flip admission narrowed to `$0` money candidates (amount exactly 0, kind recurring/sparse) — welcome/account kind-free mails with NO amount can never flip a paid row to free; (B) rollup no longer lets a last-arriving `$0` receipt collapse a merchant with real charges (pure-$0 corpora still free); (C) marketing demotion now requires SUBJECT-tier ad shapes (body CTA noise + bulk hints never demote — R27's preserved class); (D) `cadenceClear` provenPayment = strong charge artifacts only (receipt/invoice/statement words and payment-method mentions excluded); plus rail item extraction rejects prose tails (stopwords/4-word cap). 7 new tests → jest 518/518, tsc clean.
- **THE GATE — DONE (2026-09-19, v30 on device, receipts in `~/cadence_checkpoints/gate_v30_*.xml` + `docs/baselines/2026-09-19.md`):** v30 bump wiped the classified corpus (documented bump behavior); first restage re-derived a shallow-window world (209.13) → recovered via Settings → Deep Re-list (Phase K; walk exhausted page 22, 2102 ids, corpus rebuilt to 1699 rows, `spend-projection-audit: projection=189.83 legacy=189.83 delta=0.00 match=yes`, subs=106). Six-row AFTER table, all read on device: **Hightimes** recurring $59.99 Monthly, Started 09/16/2026; **RockAuto** sparse ? This month, Started 01/25/2026; **Squarespace** recurring $41.00 Monthly, Started 12/29/2024 (v29-healed date survived); **Netgate** FREE $0.00, Started 02/02/2026 — the P3 $0-license flip fired for real once the deep corpus restored the license mail (Monthly label = legacy stamp, contributes $0); **Rotaryengine** sparse $0.00, Started 01/10/2025 (stamp stays cleared); **Google Play** family card recurring $68.97 **Yearly** + $67.96 Sparse, Started 11/25/2022 (cadence healed Monthly→Yearly honestly from restored deep evidence). Guards: no twins (six brand searches each Results (1)); subs 104→106 = two honest mints from deep-restored mail (e.g. Google One $149.99 Yearly — Phase-K corpus rebuild, NOT P2/P3 flips); audit match=yes. Emulator revived by cold boot `-no-snapshot-load` (no destructive surgery); ANR handled with Wait.
- **P4 — Provider plumbing (after the gate, cheapest-first):** Graph `inferenceClassification` in `$select` · JMAP keep `htmlBody` + request `headers` · IMAP HTML part + `BODY[HEADER.FIELDS]` + `X-GM-LABELS` · Proton MIME HTML/headers hop · Tuta regex-tier only (documented).
- **R39 — parked per approved order:** derived `nextExpectedCharge` (fixes permanently-empty "Upcoming") + sorter spec (most-recent overall DEFAULT / sparse / recurring / oldest ×3 / next-billing-date) + persisted default view Active + most-recent overall. Gate: all sorts visible on device.
- **R40-A — DONE (`781a2c7`, 2026-09-19; the "major bug" the user named: the list still ordered by scan wall-clock):** `getAllSubscriptions` now orders by `COALESCE(last_received_at, start_date, created_at) DESC` — received evidence first, scan order never. New schema **v21** `subscriptions.last_received_at` written at MINT (rollup carries `lastReceived` = max corpus message date alongside `firstSeen`; `candidateToSubscription` stamps it), at REPAIR as a MAX-ONLY **standalone bookkeeping write** (`lastReceivedAdvance` else-branch) — deliberately NOT a key into the kind-guarded repair block, so a sparse candidate's newer mail date can never unlock R18/R22/R34-guarded writes onto a recurring row (8 legacy zero-write assertions updated to "no repair writes, bookkeeping-only allowed"). `nextExpectedChargeFor` projects the next charge from row evidence (renewalDate when future, else start + k×cadence; never invents a cadence) → `getUpcomingSubscriptions` no longer requires `renewal_date` (was permanently empty since scan rows never carry it). Subscriptions screen ships the R39 five sorts (Recent default / Oldest / Sparse / Recurring / Next charge) as visible chips, with persisted `subs_sort` + persisted default view `subs_default_filter` (first-run default Active; Home's nav param still wins). Tests: `subscriptionOrder.test.ts` 13 cases; jest 531/531, tsc clean. **Device gate PASSED** (JS reload via running Metro, no rebuild): sort switching visibly reorders by evidence (Recent: Wikimedia-first ↔ Oldest: 2017/2022-era rows first); sort + view survive force-stop/relaunch; Upcoming (0) correct for this corpus (no charge projects within 7d); spend $189.83 unchanged. Receipts: `docs/baselines/2026-09-19.md` rows 9–11. NOTE: A3 app-pass gate observed LIVE in this build ("Unlock Cadence", pass `r38gate1`) — re-check A3/A4 ledger status before rebuilding it.

**Session context (recovered 2026-09-18, do not re-derive):** emulator rebuilt after the 2026-09-17 locksettings wipe broke zygote (self-admitted mistake) → Sep-15 snapshot → re-onboarding; vault pass `r38gate1` (phrase words 2/7/9 = shoe/juice/tube); backup passphrase `r38evidence`; corpus `/sdcard/Download/cadence_corpus.cbak` (12.8MB, 09-18 12:46) restored: 107 subs, $484.75/mo, all six bad rows with paper-trails + source emails. The restored corpus is old-parser output — it is Phase 0's audit substrate and the "before" column, NOT proof of anything. The six-row report is session idx 8898; approved plan idx 8947/8956. Insights duplicate-key fix landed (`ee4f9d7`): name-keyed React lists need index suffix (corpus-dependent dupes).

**Standing rules:** one `run_commands` command per call, sequential; long/stream commands annotated or detached; no emulator "repairs" — snapshot recovery only, no destructive system surgery without asking; docs after gates, never instead of them; 3-strike safeguard per `.clinerules/00-core-safeguards.md`.
