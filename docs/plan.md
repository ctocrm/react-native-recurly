...
**Act next:** Wave 1 Connect status: #1 Outlook Graph proven (`e5bde4c`), #2 Google Workspace proven (Gmail API enablement; see 2026-09-02 log). Remaining: IMAP last-row; O365 fresh sign-in needs `M365_USER`/`M365_PASS` in `.env`; Outlook stored token can go corrupt/stale (`IDX14100`) — no refresh-before-list, separate bug. Icon hops 1–5, the xAI compound-label hop, and Cadence identity are closed. Do not restore `063ac4b`. Do not start the `x.` social-exclusion hop — this crawl used `official=x.ai`, not Twitter.
