...
| 2026-09-03 | **Clean-slate visual run surfaced R10: full-mailbox scan OOM-crashes the app.** With all subscriptions deleted (45 via scripted UI deletion + 1 manual confirm) and caches cleared, the fresh scan refreshed the expired Workspace token (13:49:05→07, silent) then **OOM-crashed at 13:49:25** — `java.lang.OutOfMemoryError` 192MB heap in FabricEventDispatcher (crash buffer, pid 2394). Root cause class: the Workspace Gmail full-fetch path has NEVER run to completion — every earlier attempt died at the list-time 401 that R1 fixed; the first complete run blows the heap (per-message `format=full` accumulation across pages). The crash masks the visual gates (R2/R3/R7 import behavior, R4 pickers) — no rows imported, no error surfaced to UI; app auto-restarted to an empty idle tab. NOT yet fixed: needs the scan pipeline paging/accumulation audited (`runIncrementalScan` ↔ `createGmailFetcher` batching) before the visual pass can complete. All other fixes remain installed; deletion of subscriptions did NOT touch mailbox tokens. |
