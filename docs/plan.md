...
**Status:** Phases **1–5.5 complete**. **MAJOR FIX (Tranches A–F) complete on-device.** **UI Improvements Phase 0–3 complete** (`ba66478`, `e4e383e`, `77a83bb`, `913b08f`). **UI Phase 4 (email account scan) is NOT done.** H1–H5 code is in (`d83a539`, `34f2fee`, `f400c79`, `2ab2cbb`) and the x86_64 APK is installed on emulator. Remaining: live OAuth when mail client ids exist; Proton OpenPGP body decrypt; Tuta full KDF + on-device decrypt. Phase 5 starts only after Phase 4 can import rows. Training frozen.
...
- [x] **H1 — Honesty (catalog + copy + tests).** Drop Yahoo/AOL branded OAuth. IMAP note without Proton/Tuta. Settings/IMAP-sheet copy factual. Tests must not lock Yahoo/AOL as branded OAuth. (`d83a539`)
- [x] **H2 — Native IMAP socket** (not JS-only Expo). Tracked Expo plugin copies Kotlin `MailImap` SSL IMAPS into prebuild. Public hosts only (iCloud / Yahoo / AOL / custom). Still not Proton/Tuta. (`34f2fee`)
- [x] **H3 — HTTPS completeness**: Zoho Mail HTTP search fetcher; Fastmail JMAP subject-union query; Office 365 live-scan via Graph. Mail client ids still fall back to Drive/OneDrive env keys when dedicated keys are absent. (`f400c79`)
- [x] **H4 — Proton client API (headers).** Password (+ 2FA) sheet. Native `MailProton`: SRP + `auth/v4` + `mail/v4` list. **Not IMAP, not Bridge.** Subject/From/Date go to the shared classifier. **OpenPGP body decrypt is not in this cut** — ciphertext bodies are not parsed as money. (`2ab2cbb`)
- [x] **H5 — Tuta client protocol (headers, GPL-safe).** Password sheet. Native `MailTuta`: SaltService + SessionService + REST list. **Not IMAP.** No copy of the GPLv3 client. KDF is a first-cut verifier (not full Argon2id/bcrypt); **on-device decrypt is not in this cut.** Honest “no public docs / unsupported.” (`2ab2cbb`)
...
- After H1: tests assert branded OAuth rows = Gmail / Workspace / Outlook / Office365 / Zoho / Fastmail; IMAP last; **no** Yahoo/AOL branded OAuth; IMAP live-scan false. Proton/Tuta branded **client-REST** rows land in H4/H5 (not in H1).

#### Visual gate (skill loop) — after H1 copy, not before

1. Settings → Email scan list visible above the nav overlay.
2. After H1: branded OAuth rows are Gmail, Workspace, Outlook, Microsoft 365, Zoho, Fastmail. Each starts that provider’s OAuth (not an IMAP form).
3. IMAP row opens host/user/password (or app password). Copy names **iCloud, Yahoo, AOL, custom** — **not** Proton/Tuta.
4. After H1, copy must **not** say Proton/Tuta “use IMAP.” After H4/H5, branded Proton/Tuta rows open a **password** sheet (not OAuth, not IMAP).
...
6. Live order when mail client ids exist: Gmail connect + empty recurring Scan first; then Workspace full scan; Outlook full scan. Proton/Tuta live tests wait for H4/H5. Never IMAP-test them.
...
| 2026-08-20 | **UI Phase 4 is NOT done.** Proton/Tuta are not IMAP (and not drop). Phone path is first-party **client REST + on-device decrypt**: Proton H4 (SRP + OpenPGP + `mail/v4`, [go-proton-api](https://github.com/ProtonMail/go-proton-api)); Tuta H5 (FAQ-invited `/rest/...`, no public docs yet, GPL-safe). Yahoo/AOL branded OAuth closed. No Bridge/tunnels. Remedy: H1 copy, H2 IMAP socket, H3 HTTPS fetchers, H4 Proton, H5 Tuta. Plan-only. |
| 2026-08-20 | **H1–H5 landed; Phase 4 still not complete.** H1 `d83a539` honest catalog. H2 `34f2fee` native IMAPS SSL socket. H3 `f400c79` Zoho/Fastmail/Office365 HTTPS. H4/H5 `2ab2cbb` Proton/Tuta password sheets + native REST (headers only; no OpenPGP/Tuta decrypt). x86_64 APK installed on emulator. Remaining: live OAuth with mail client ids; Proton body decrypt; Tuta full KDF + decrypt.                                                     |
