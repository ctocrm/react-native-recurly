...
- I1: scan-progress event surface from the leg loop (legs done/total, staged counts via the existing pacer ticks) + `useScanProgress` hook.
- I2: Home scan button goes fire-and-forget (no full-time spinner/disable); persistent "Scanning… leg k/N" pill; completion toast instead of `Alert.alert`.
- I3: per-mailbox errors surface in the pill/toast (R2 parity), not modals.
