...
- User bar (Aug 2026): ~30–50% better _perceived_ quality — slight PSNR wins are not enough
...
loss = MAE
     ...
     # + 0.08 * sobel_edge_loss     # OFF default; opt-in --edge only
     #   (tf.image.sobel_edges → MirrorPad multi-GPU crashes on many sizes;
     #    brand-safe reintro regression 2026-08-08 full matrix)
...
- **Bug to never repeat:** zeroing a head _after_ depth_to_space (body learned garbage HR)
...
Progressive 2× (LapSRN, ProSR, waifu2x, Real-ESRGAN zoo) is still right for _photo sharpness_.  
...
- Unattended mobile **logos**: our bilin-first freeze is the better _product_ fit
...
_Not shipped. Explore only after freeze + visual OK on current path._
...
**B — waifu2x-style cascade of _our_ 2× cells**

```text
16 →2× hybrid→ 32 →2× hybrid→ 64 →2× hybrid→ 128 → …
```

- Right way to import waifu2x _structure_ on-device
