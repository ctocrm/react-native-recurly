...
26. **A per-leg catch that swallows without logging makes a leg invisible (2026-09-08):** in the R20 release scan gate the workspace leg produced `fetcherFor` + token refresh at 04:13:14 and by 04:13:20 the NEXT leg was logging — no summary, no error line, anywhere in the log. The leg's Graph error died inside `importFromConnectedMailboxes`' per-leg `try/catch` (`scanConnected.ts`) with no log statement. Detection heuristic: **count `fetcherFor` lines vs summary lines — every fetcherFor without a matching summary is a silently swallowed leg.** Fix direction (backlog, not yet done): log the caught error inside the per-leg catch. (Amendment 2026-09-16: the catch now logs `console.warn("[MailScan] leg failed …")` + `scanProgressLegError` + `errors.push` in `scanConnected.ts` — verified in source and superseded as backlog.) Related same-day artifact: replacing the APK (`adb install -r`) while the fire-and-forget icon-crawl chain was still draining killed in-flight persists (icon auto-assignments like Zoom's) and the live-spend optimism (release live $327.08 vs cold debug re-read $177.08 of the same committed rows) — before attributing a release-vs-cold-read delta, check whether the release process was killed mid-chain.
...
40. **Buckets are a derived cache, not data — and a schema check cannot prove a strip-list fix (2026-09-16):** two follow-ups from the gate. (a) Connecting a mailbox RESTAGES the spend projection: `merchant_day_actuals` is dropped and recomputed from the CURRENT corpus, so a fresh import of a corpus-less backup reads Home $0.00→subs-only $280.95 until a rescan rebuilds the corpus. Never treat a post-import Home number as stable if any mailbox event fires; the SSOT is corpus+subscriptions, buckets are disposable. (b) The strip list works by `DELETE FROM exp.<table>` — it removes ROWS, not the TABLE, so "does mail_messages exist in the file" proves nothing; only a row-count check with a NON-empty corpus discriminates the fix. Offline verification recipe that worked: `python3 -m venv /tmp/sqlcv && /tmp/sqlcv/bin/pip install sqlcipher3-binary`, then `PRAGMA key='<pass>'` + row counts (SQLCipher 4 defaults match the app: PBKDF2-SHA512 256k). Device-driving notes from the same run: the Google consent "Unmatched Route" flash is cosmetic (token exchange completes; "Go back" recovers); the Settings bottom-sheet buttons live BEHIND the nav pill at some scroll positions — a uiautomator tap at clipped bounds hits the pill, so scroll the section clear (center-y < ~2100) before tapping; `keyevent 111` can cancel a sheet whose field never took focus; `input text` occasionally doubles keystrokes into a masked field (verify the dot count before submitting).


41. **A wall-clock default in a mint path becomes "the data" within one scan — and survives a month of audits (2026-09-19):** `candidateToSubscription` (the very first emailscan commit, `2dde162` 2026-08-18) wrote `startDate: new Date().toISOString()`, so every scanned row's Started date was the SCAN day. It survived R18-R37 because (a) the test fixtures encoded scan-day stamps as normal (`startDate: "2026-09-07..."` = that day's scan), (b) LESSONS never recorded a Started-date check, and (c) the R38 P0 audit SAW "Started 09/16/2026" on all six rows, recorded it as "UNIVERSAL: one scan minted them all", and moved on — normalizing the symptom instead of flagging the impossibility (a May receipt cannot start a subscription in September). Fix (`df7b98f`+`0229410`): rollup carries `firstSeen` (min corpus `message.date`), mint uses it, `startDateRepair` heals scan-day stamps min-only for every matched row. Rules: (1) any date shown to the user must be traceable to provider data (message dates, header dates) — a wall-clock default in a mint/derive path is a bug unless the field is genuinely "when the user did X"; (2) an audit that records the same surprising value on every row must ask WHY before writing it down; (3) device verification of a date fix needs history that predates the recency window — run the deep re-list, or the heal has nothing to heal from.

42. **One physical mailbox = one leg — a duplicate provider leg is a twin-mint machine (2026-09-19):** david@bohbotweb.com was connected BOTH as Google Workspace and as Gmail (same account, two OAuth legs, same mail). Every scan then classified the same emails under two mailboxIds, minting a twin row per merchant per leg (103 duplicate rows at peak). The R37 doctrine "two mailboxes is two honest rows" is correct ONLY for distinct accounts — provider duplication of one account is a config error, not two mailboxes. Detection tells: every merchant has exactly 2 rows with adjacent names differing only in paymentMethod; subs count ~= 2× the honest count; the twin flood survives restores because the corpus carries both key-families. Recovery that WORKS: the app's own UI path (Remove mailbox → "Remove subscriptions too") deletes the leg's rows AND `clearMailboxAsync` purges its mail_messages — after which the leg's candidates can never rebuild (the rollup reads stored messages). Verify with a single-leg re-scan: re-import count must be ~0-2, not the twin count. Also: check the mailbox IDENTITY (the address) before adding a leg, not just the provider row. Related environmental lessons same night: post-crash emulator revival = cold boot `-no-snapshot-load` from the surviving userdata qcow2 (quickboot snapshot is stale); after a guest reboot `input tap` can silently die while `input swipe X Y X Y 300` still registers (re-learned LESSONS 37's mechanic); adb reverse is wiped by guest reboots — a dev client that suddenly loads JS from assets is missing its metro reverse.
